...
public class Box2D extends Object2D implements Renderable
{
	...
	{
		this( new Vector2f( box.pos ), new Vector2f( box.getSize() ) );
	}
	...
	public final boolean contains( final Box2D box )
	{
		...
		for ( int x = 0; x < DIM; x++ )
		{
			...
			if ( b < p || b + get( box.getSize(), x ) > p + get( getSize(), x ) )
			{
				return false;
			}
		}
	...
	public final Box2D intersection( final Box2D box )
	{
		...
		for ( int x = 0; x < DIM; x++ )
		{
			...
			set( pos, x, max( p, b ) );
			set( size, x, min( p + get( getSize(), x ), b + get( box.getSize(), x ) ) - get( pos, x ) );
		}
	...
	public final boolean intersects( final Box2D box )
	{
		...
		for ( int x = 0; x < DIM; x++ )
		{
			...
			if ( b >= p + get( getSize(), x ) || b + get( box.getSize(), x ) <= p )
			{
				return false;
			}
		}
	...
	public final Box2D union( final Box2D box )
	{
		...
		for ( int x = 0; x < DIM; x++ )
		{
			...
			set( pos, x, min( p, b ) );
			set( size, x, max( p + get( getSize(), x ), b + get( box.getSize(), x ) ) - get( pos, x ) );
		}
