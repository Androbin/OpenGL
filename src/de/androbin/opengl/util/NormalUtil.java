...
public final class NormalUtil {
  ...
  public static Vector3f getNormal( final boolean ff, final Vector3f dest, final Vector3f temp,
      final Vector3f ... v ) {
    return ff ? getNormalCW( dest, temp, v ) : getNormalCCW( dest, temp, v );
  }
  
  public static Vector3f getNormalCW( final Vector3f dest, final Vector3f temp,
      final Vector3f ... v ) {
    final Vector3f v1 = Vector3f.sub( v[ 2 ], v[ 1 ], temp );
    final Vector3f v2 = Vector3f.sub( v[ 1 ], v[ 0 ], dest );
    return Vector3f.cross( v1, v2, dest ).normalise( dest );
  }
  
  public static Vector3f getNormalCCW( final Vector3f dest, final Vector3f temp,
      final Vector3f ... v ) {
    final Vector3f v1 = Vector3f.sub( v[ 0 ], v[ 1 ], temp );
    final Vector3f v2 = Vector3f.sub( v[ 1 ], v[ 2 ], dest );
    return Vector3f.cross( v1, v2, dest ).normalise( dest );
  }
  
  public static Vector3f[] getNormals( final boolean ff, final Vector3f dest, final Vector3f temp,
